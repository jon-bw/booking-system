...
function TenantPageContent({ tenantSlug }) {
  ...
  const wrapperStyle = {
    ...
    fontFamily: themeVars?.['--font-body'] ?? 'Space Grotesk, sans-serif',
  };

  if (loading || themeLoading) {
    return <div className="themed-content min-h-screen flex items-center justify-center" style={wrapperStyle}><p className="font-mono uppercase text-sm animate-pulse">Loading</p></div>;
  }

  if (error || blocks.length === 0) {
    return (
      <div className="themed-content min-h-screen flex flex-col items-center justify-center space-y-6 px-6" style={wrapperStyle}>
        <h1 className="font-display text-4xl" style={wrapperStyle}>{error ? 'Page Not Available' : 'Page Coming Soon'}</h1>
        <p className="font-mono uppercase text-xs" style={{ color: themeVars?.['--text-muted'] ?? '#6b7280' }}>{error || 'This page is currently being built.'}</p>
        <Link to="/" className="font-mono uppercase rounded-full border px-6 py-3 text-sm hover:bg-surface transition-colors" style={{ borderColor: themeVars?.['--border'] ?? '#333333' }}>Return Home</Link>
      </div>
    );
  }
  ...
  const renderBlock = (block) => {
    ...
    if (!Component) return <div key={block.id} className="border-b py-16 text-center font-mono text-xs" style={{ borderColor: themeVars?.['--border'] ?? '#333333' }}>Unknown block: {block.blockType}</div>;
  ...
  return (
    <div className="themed-content min-h-screen" style={wrapperStyle}>
